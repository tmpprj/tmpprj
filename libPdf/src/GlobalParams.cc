...
std::auto_ptr<GlobalParams> globalParams;
...
GBool GlobalParams::parseKey(GString *modKeyStr, GString *contextStr,
			     ...
			     char *cmdName,
                 GList *tokens, GString *fileName, int line) {boost::ignore_unused_variable_warning( tokens );
  char *p0;
...
void GlobalParams::addSecurityHandler(XpdfSecurityHandler *handler) {boost::ignore_unused_variable_warning( handler );
#ifdef ENABLE_PLUGINS
  lockGlobalParams;
...
XpdfSecurityHandler *GlobalParams::getSecurityHandler(char *name) {boost::ignore_unused_variable_warning( name );
#ifdef ENABLE_PLUGINS
